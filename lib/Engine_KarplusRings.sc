...
Engine_KarplusRings : CroneEngine {
  ...
  var lpf_freq = 3000;
	...
	alloc {
		...
		SynthDef("KarplusRings", {arg out, amp = amp, freq = freq, decay = decay, coef = coef, lpf_freq = lpf_freq, lpf_gain = lpf_gain, bpf_freq = bpf_freq, bpf_res = bpf_res;
			var env, snd;
		...
		this.addCommand("hz", "f", { arg msg;
			var val = msg[1];
            Synth("KarplusRings", [\out, context.out_b, \freq,val,\amp,amp,\decay,decay,\coef,coef,\lpf_freq,lpf_freq,\lpf_gain,lpf_gain,\bpf_freq,bpf_freq,\bpf_res,bpf_res], target:pg);
		});
